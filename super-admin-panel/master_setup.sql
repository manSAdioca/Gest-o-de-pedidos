...
-- 2. Garante que a coluna de logo exista na tabela tenants
DO $$ 
BEGIN 
...
-- 3. Cria a Função RPC Segura para o Painel criar as lojas
...
-- 4. Cria o Bucket "logos" publico
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types) 
VALUES ('logos', 'logos', true, null, null) 
ON CONFLICT (id) DO UPDATE SET public = true;

DROP POLICY IF EXISTS "Leitura publica logos" ON storage.objects;
CREATE POLICY "Leitura publica logos" ON storage.objects FOR SELECT TO public USING (bucket_id = 'logos');

DROP POLICY IF EXISTS "Escrita logos" ON storage.objects;
CREATE POLICY "Escrita logos" ON storage.objects FOR INSERT TO public WITH CHECK (bucket_id = 'logos');

-- 5. FORÇA A ATUALIZAÇÃO DO CACHE DA API (MUITO IMPORTANTE)
